...
public class EventService {
	...
	@RequestMapping("/getInvocationGraph/{taskId}/{constantNodeSize}")
	public String getInvocationGraph(@PathVariable Long taskId, @PathVariable boolean constantNodeSize) {
		...
		Map<String, Integer> numberOfEdgesBetween2Types = new HashMap<>();
		Map<String, Integer> numberOfEventsOnTypes = new HashMap<>();
		...
		for (Map.Entry<String, Integer> stringIntegerEntry: numberOfEventsOnTypes.entrySet()) {
			...
			if(constantNodeSize) {
				html.append("{data:{id:'").append(element.getKey()).append("',mySize: ")
						.append(10).append("}},\n");
			} else {
				html.append("{data:{id:'").append(element.getKey()).append("',mySize: ")
		...
		for (Map.Entry<String, Integer> stringIntegerEntry : numberOfEdgesBetween2Types.entrySet()) {
			...
			html.append("{data:{id:'").append(typeNames[0]).append(",").append(typeNames[1])
